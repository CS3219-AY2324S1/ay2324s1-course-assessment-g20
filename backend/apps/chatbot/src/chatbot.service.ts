import { ChatbotRequest, ChatMessage, ChatMessagesResponse } from '@app/microservice/interfaces/chatbot';
import { QuestionServiceClient, QUESTION_SERVICE_NAME } from '@app/microservice/interfaces/question';
...
import { CollaborationServiceClient, COLLABORATION_SERVICE_NAME } from '@app/microservice/interfaces/collaboration';
...
3. Come up with a solution for you`
  ...
  async createQuery(request: ChatbotRequest): Promise<ChatHistory> {
    ...
    const questionId = await (await firstValueFrom(this.collaborationService.getQuestionIdFromSessionId({ id: sessionId }))).questionId;
    ...
    if (chatHistory == null || chatHistory.messages.length == 0) {
      const question = await firstValueFrom(this.questionService.getQuestionWithId({ id: questionId }))

      filteredChatHistory = await this.getChatbotReplyForNewSession(query, userSolution, question.description, language, sessionId, userId);
    } else {
      filteredChatHistory = await this.getChatbotReplyForCurrentSession(query, userSolution, chatHistory, language);
    }
  ...
  async getChatbotReplyForCurrentSession(query: string, userSolution: string, chatHistory: ChatHistory, language: string): Promise<ChatHistory> {
    const messages = chatHistory.messages;

    // Prompt to update the context of the chatbot
    const languageAndUserSolutionContext: ChatMessage = this.generateUserLanguageAndSolutionContext(language, userSolution);
    ...
    return this.pushMessagesToChatHistory(chatHistory.sessionId, chatHistory.userId, [languageAndUserSolutionContext, userQuery, openaiResponse]);
  }

  async getChatbotReplyForNewSession(query: string, userSolution: string, description: string, language: string, sessionId: string, userId: string): Promise<ChatHistory> {

    // Prompt to give context to gpt for our application and what we want to do
    const applicationContext: ChatMessage = this.generateApplicationContext(description);
    const languageAndUserSolutionContext: ChatMessage = this.generateUserLanguageAndSolutionContext(language, userSolution);
    const messages = [applicationContext, languageAndUserSolutionContext, ChatbotService.greetingMessage, { role: 'user', content: query }];
  ...
  async getChatHistoryForFrontend(sessionId: string, userId: string): Promise<ChatMessagesResponse> {
    const chatHistory = await this.getChatHistory(sessionId, userId);
    if (chatHistory === null) {
  ...
  private filterChatHistory(chatHistory: ChatHistory): ChatHistory {
    chatHistory.messages = chatHistory.messages.filter((message) => message.role !== 'system');
    return chatHistory;
  }

  private getChatHistory(sessionId: string, userId: string): Promise<ChatHistory> {
  ...
  private addNewChatHistory(sessionId: string, userId: string, messages: ChatMessage[]): Promise<ChatHistory> {
    return this.chatHistoryModel.findOneAndUpdate({ sessionId, userId }, { $set: { messages } }, { upsert: true, new: true }).exec();
  }


  private async getOpenAIResponse(messages: ChatMessage[]): Promise<ChatMessage> {
  ...
  private generateUserLanguageAndSolutionContext(language: string, userSolution: string): ChatMessage {
    return {
      role: 'system',
